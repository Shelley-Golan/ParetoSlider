...
def pipeline_with_logprob(
    ...
    if preference is not None:
        if preference.ndim != 2:
            raise ValueError(f"preference must be 2D (B, pref_dim), got {tuple(preference.shape)}")

        # Move to device (keep float32 outside model; model will cast)
        ...
        if preference.shape[0] == batch_size:
            preference = preference.repeat_interleave(num_images_per_prompt, dim=0)
        elif preference.shape[0] != batch_size * num_images_per_prompt:
            raise ValueError(
                f"preference batch mismatch: got {preference.shape[0]}, expected {batch_size} or {batch_size * num_images_per_prompt}"
            )
    ...
    if not flux:
        ...
        if self.do_classifier_free_guidance:
            ...
            # For Panacea: duplicate preference for CFG (same preference for uncond & cond)
            if preference is not None:
                preference = torch.cat([preference, preference], dim=0)
    else:
        (
            prompt_embeds,
            pooled_prompt_embeds,
    ...
    def v_pred_fn(z, sigma):
        if not flux:
            ...
            if preference is not None:
                transformer_kwargs["preference"] = preference
            ...
            if preference is not None:
                transformer_kwargs_flux["preference"] = preference
            noise_pred = self.transformer(**transformer_kwargs_flux)[0]
