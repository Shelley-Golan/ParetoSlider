...
STYLE_PROMPT = """You are an expert visual style evaluator.

Your task is to judge how faithfully the image matches the TARGET STYLE described below.

Important:
- Focus on STYLE, not whether you personally like the image.
- Do not reward the image just for being high quality.
- Do not punish the image for content differences unless they interfere with judging the style.
- If the image matches the prompt content but not the target style, give a low style score.
- If the image is highly stylized but in the wrong style, give a low style score.
...
Evaluate the image on these three dimensions from 1 to 5:

1. Style Adherence
How well does the image express the defining visual characteristics of the target style?

2. Style Consistency
How internally consistent is the style across the whole image, without mixed or conflicting style cues?

3. Style Specificity
How specifically does the image match this target style rather than a generic stylized look?
...
4 = strong match
5 = excellent, highly faithful match

Return your answer in valid JSON only with this schema:
{{
  "reasoning": "brief but specific explanation",
  "style_adherence": <1-5 integer>,
  "style_consistency": <1-5 integer>,
  "style_specificity": <1-5 integer>,
  "final_score": <1-5 integer>
}}

Set final_score as your overall style faithfulness judgment, not the simple average if one dimension is a clear failure.
"""
