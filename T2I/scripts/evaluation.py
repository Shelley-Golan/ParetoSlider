...
def main(args):
    ...
    if args.num_samples and args.num_samples < len(dataset):
        ...
        if is_main_process(rank):
            print(f"Subsampled {args.num_samples} prompts (seed={args.seed})")
    ...
    prompts_json_path = os.path.join(os.path.dirname(args.output_dir), "prompts.json")
    ...
    for batch in tqdm(dataloader, desc=f"Evaluating (Rank {rank})", disable=not is_main_process(rank)):
        ...
        for i in range(current_batch_size):
            ...
            image_path = os.path.join(args.output_dir, f"{sample_idx}.png")
    ...
    if is_main_process(rank):
        ...
        # Write prompts.json to parent dir for score_generated_images.py --no_cond
        prompts_map = {str(r["sample_id"]): r["prompt"] for r in flat_results}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--generate_only", action="store_true", help="Only generate and save images, skip reward scoring.")
