...
from flow_grpo.diffusers_patch.train_dreambooth_lora_sd3 import encode_prompt as encode_prompt_sd3

# Flux2 imports (conditional - may not be available in all diffusers versions)
try:
    from diffusers import Flux2KleinPipeline, Flux2Pipeline
    from flow_grpo.diffusers_patch.train_dreambooth_lora_flux2 import encode_prompt as encode_prompt_flux2
    FLUX2_AVAILABLE = True
except ImportError:
...
class ShareGPTPromptDataset(Dataset):
    def __init__(self, dataset, split="train", test_ratio=0.02, seed=42):
        dataset_path = Path(dataset)
        self.file_path = str(dataset_path if dataset_path.is_file() else dataset_path / "text_to_image.json")
        with open(self.file_path, "r", encoding="utf-8") as f:
            items = json.load(f)
        ...
        if split == "train":
            ...
            indices = indices[split_idx:]
        self.prompts = [items[idx]["input_prompt"] for idx in indices]
...
def compute_text_embeddings(prompt, text_encoders, tokenizers, max_sequence_length, device, base_model="sd3", pipeline=None):
    with torch.no_grad():
        if base_model in ("flux2", "flux2base", "flux2dev"):
            # Flux2 Klein/Klein Base/Dev uses pipeline's encode_prompt
            prompt_embeds, text_ids = encode_prompt_flux2(pipeline, prompt, device, max_sequence_length=max_sequence_length)
            prompt_embeds = prompt_embeds.to(device)
            return prompt_embeds, None, text_ids
        else:
            # SD3 uses dual CLIP + T5
            prompt_embeds, pooled_prompt_embeds = encode_prompt_sd3(text_encoders, tokenizers, prompt, max_sequence_length)
            prompt_embeds = prompt_embeds.to(device)
...
def eval_fn(
    ...
    is_flux2 = config.base_model in ("flux2", "flux2base", "flux2dev")
    model_type = "flux2" if is_flux2 else "sd3"
    ...
    neg_prompt_embed, neg_pooled_prompt_embed, neg_text_ids = compute_text_embeddings(
        ...
        base_model=config.base_model, pipeline=pipeline
    )
    ...
    for test_batch in tqdm(
        ...
        if current_batch_size < len(sample_neg_prompt_embeds):  # Handle last batch
            ...
            if sample_neg_pooled_prompt_embeds is not None:
                current_sample_neg_pooled_prompt_embeds = sample_neg_pooled_prompt_embeds[:current_batch_size]
            else:
                current_sample_neg_pooled_prompt_embeds = None
        else:
        ...
        with torch_autocast("cuda", enabled=(config.mixed_precision in ["fp16", "bf16"]), dtype=mixed_precision_dtype):
            with torch.no_grad():
                pipeline_result = pipeline_with_logprob(
                    pipeline,
                ...
                # Unpack result based on model type
                if is_flux2:
                    images, _, _, _, _ = pipeline_result
                else:
                    images, _, _ = pipeline_result
...
def main(_):
    ...
    if is_main_process(rank):
        ...
        wandb.init(project="nft-sd3", name=config.run_name, config=config.to_dict(), dir=log_dir)
    logger.info(f"\n{config}")
    ...
    if config.base_model in ("flux2", "flux2base", "flux2dev"):
        if not FLUX2_AVAILABLE:
            raise ImportError("Flux2 pipeline not available. Please update diffusers.")
        if config.base_model == "flux2dev":
            pipeline = Flux2Pipeline.from_pretrained(config.pretrained.model)
        else:
            # Klein (distilled) and Klein Base (undistilled) share the same pipeline
            pipeline = Flux2KleinPipeline.from_pretrained(config.pretrained.model)
    ...
    for te in text_encoders:
        te.to(device, dtype=text_encoder_dtype)
    ...
    if config.use_lora:
        if config.base_model in ("flux2", "flux2base", "flux2dev"):
            # Flux2 attention module names (same for Klein, Klein Base, and Dev)
            target_modules = [
                "attn.to_q",
                "attn.to_k",
                "attn.to_v",
    ...
    elif config.prompt_fn == "sharegpt":
        train_dataset = ShareGPTPromptDataset(config.dataset, "train")
        test_dataset = ShareGPTPromptDataset(config.dataset, "test")
    else:
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        for i in tqdm(
            ...
            with torch_autocast("cuda", enabled=enable_amp, dtype=mixed_precision_dtype):
                with torch.no_grad():
                    current_neg_pooled = sample_neg_pooled_prompt_embeds[: len(prompts)] if sample_neg_pooled_prompt_embeds is not None else None
                    pipeline_result = pipeline_with_logprob(
                        pipeline,
                        prompt_embeds=prompt_embeds,
                        pooled_prompt_embeds=pooled_prompt_embeds,
                        negative_prompt_embeds=sample_neg_prompt_embeds[: len(prompts)],
                        negative_pooled_prompt_embeds=current_neg_pooled,
                    ...
                    # Unpack result based on model type
                    if is_flux2:
                        images, latents, latent_image_ids, sample_text_ids, _ = pipeline_result
                    else:
                        images, latents, _ = pipeline_result
                        latent_image_ids = None
                        sample_text_ids = None
            transformer_ddp.module.set_adapter("default")
            ...
            if is_flux2:
                sample_data["text_ids"] = sample_text_ids
                sample_data["latent_image_ids"] = latent_image_ids
        ...
        for inner_epoch in range(config.train.num_inner_epochs):
            ...
            for i, train_sample_batch in tqdm(
                ...
                if config.sample.guidance_scale > 1.0 and not is_flux2:
                    # SD3 with classifier-free guidance
                    embeds = torch.cat(
                    ...
                    # Flux2 doesn't use CFG in the same way, or no CFG
                    embeds = train_sample_batch["prompt_embeds"]
                    pooled_embeds = train_sample_batch.get("pooled_prompt_embeds", None)
                ...
                for j_idx, j_timestep_orig_idx in tqdm(
                    ...
                    with torch_autocast("cuda", enabled=enable_amp, dtype=mixed_precision_dtype):
                        ...
                        # Prepare transformer kwargs based on model type
                        if is_flux2:
                            # Flux2 uses different forward args
                            timestep_scaled = train_sample_batch["timesteps"][:, j_idx] / 1000.0
                            guidance_val = torch.full([xt.shape[0]], config.sample.guidance_scale, device=device, dtype=torch.float32)
                            transformer_kwargs = {
                                "hidden_states": xt,
                                "timestep": timestep_scaled,
                                "encoder_hidden_states": embeds,
                                "img_ids": train_sample_batch["latent_image_ids"],
                        ...
                        with torch.no_grad():
                            # prediction v (old adapter)
                            old_prediction = transformer_ddp(**transformer_kwargs)[0].detach()
                        transformer_ddp.module.set_adapter("default")

                        # prediction v (default adapter)
                        forward_prediction = transformer_ddp(**transformer_kwargs)[0]

                        with torch.no_grad():  # Reference model part
                            # For LoRA, disable adapter.
                            if config.use_lora:
                                with transformer_ddp.module.disable_adapter():
                                    ref_forward_prediction = transformer_ddp(**transformer_kwargs)[0]
                                transformer_ddp.module.set_adapter("default")
